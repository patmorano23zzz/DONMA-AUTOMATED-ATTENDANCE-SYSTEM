...
:root{--blue:#1a56db;--blue-d:#1044b8;--blue-l:#dbeafe;--green:#16a34a;--red:#dc2626;--radius:16px;--shadow:0 8px 32px rgba(26,86,219,.13)}
@keyframes fadeUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
@keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.08)}}
@keyframes spin{to{transform:rotate(360deg)}}
@keyframes scanLine{0%{top:0}100%{top:100%}}
@keyframes popIn{from{opacity:0;transform:scale(.85)}to{opacity:1;transform:scale(1)}}
body{font-family:'Segoe UI',system-ui,sans-serif;min-height:100vh;background:#f0f4ff;display:flex;flex-direction:column}
.reg-header{background:linear-gradient(135deg,#0d1b2a,#1a3a6b);padding:1rem 1.5rem;display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap}
...
.step-item.active .step-circle{border-color:var(--blue);background:var(--blue);color:#fff;box-shadow:0 0 0 4px rgba(26,86,219,.2)}
...
.reg-card-header{background:linear-gradient(135deg,var(--blue),#3b82f6);padding:1.5rem 1.75rem 1.25rem;color:#fff;text-align:center}
...
.reg-btn-primary{background:linear-gradient(135deg,var(--blue),#3b82f6);color:#fff}
