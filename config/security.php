...
function csrf_verify(): void {
    ...
    if (!hash_equals(csrf_token(), $token)) {
        // For API endpoints (JSON), return 403 JSON
        $accept = $_SERVER['HTTP_ACCEPT'] ?? '';
        if (str_contains($accept, 'application/json') || str_contains($_SERVER['CONTENT_TYPE'] ?? '', 'application/json')) {
            http_response_code(403);
            header('Content-Type: application/json');
            die(json_encode(['error' => 'Invalid CSRF token. Please reload the page and try again.']));
        }
        // For regular form pages — flash an error and redirect back
        if (session_status() === PHP_SESSION_NONE) session_start();
